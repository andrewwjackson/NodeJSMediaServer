...
function streamBuffer(res, req, buffer, contentType, modstring) {
  try {
    if (buffer.length > 0) {
      ...
      res.setHeader(
        "Cache-Control",
        config.caching.cachability +
          ", max-age=" +
          config.caching.maxage.toString()
      );
      res.setHeader(
        "expires",
        dateFormat(expires, "ddd, dd mmm yyyy HH:MM:ss") + " GMT"
      );
      res.setHeader(
        "last-modified",
        dateFormat(moddate, "ddd, dd mmm yyyy HH:MM:ss") + " GMT"
      );
...
async function doGarbageCollection(callback) {
  //GC: check memory usage and recycle if needed
  if (process.memoryUsage().rss >= maxmem) {
    process.send(endpointid + ":recycle:" + process.pid);
    setTimeout(function() {
      process.send(endpointid + ":restart:" + +process.pid);
    }, 500);
  }
}
...
function requestHandler(req, res) {
  ...
  query = qs.parse(
    req.url.indexOf("?") > -1
      ? req.url.substr(req.url.indexOf("?")).length > 1
  ...
  var dbconfig =
    query.db !== undefined
      ? config.sql.conn[query.db] !== undefined
        ? config.sql.conn[query.db.toLowerCase()]
        : config.sql.conn[config.sql.default.toLowerCase()]
      : config.sql.conn[config.sql.default.toLowerCase()];
  ...
  fs.exists(localpath, function(exists) {
    if (exists && mediaid.length === 0 && config.server.fsEnabled === true) {
      ...
      console.log(
        `Looking for ${mediaid.length > 0 ? mediaid : action} in MediaLibrary`
      );
      s.getMedia(action, maxmem, dbconfig, mediaid, function(
        recordsets,
        err,
        sql
      ) {
        ...
        if (nulldata === true) {
          console.log(
            `DB Blob too big (db source uses 2x memsize): ${blobsize}`
          );
          doGarbageCollection();
          process.send(endpointid + ":error");
          res.statusCode = 500;
